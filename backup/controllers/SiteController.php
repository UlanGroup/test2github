...
class SiteController extends Controller
{
    ...
    public function behaviors()
    {
        ...
        $behaviors['authenticator'] = [
            ...
            'optional' => ['login', 'registration', 'index'],
        ];
...
//
//        $user->password_hash = Yii::$app->security->generatePasswordHash('tradingPass117');
//        if (!$user->save()) {
...
//        }
        ...
        if ($model->load(Yii::$app->getRequest()->getBodyParams(), '') && $model->login()) {
            ...
            if (empty($user)) {
                Error::add('login нет юзера');
                return ['alert' => ['msg' => 'Нет такого пользователя', 'type' => 'error']];
            }

            $token = $jwt
                ->builder()
                // Configures the issuer (iss claim)
                ->issuedBy('https://mmvb.studiobaraban.ru')
                // Configures the audience (aud claim)
                ->permittedFor('https://mmvb.studiobaraban.ru')
            ...
            return ['alert' => ['msg' => 'Неправильный логин или пароль', 'type' => 'error']];
        }

        $model->validate();

        return ['token' => $token ?? null, 'profile' => $user->userArray(), 'menu' => Menu::getMenu($user->role)];
    }
